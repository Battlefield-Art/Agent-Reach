...
|------|---------|-----------|-------|
| 🌐 **网页** | 阅读任意网页 | — | 无需配置 |
| 🌐 **Web (anti-bot)** | Cloudflare 绕过、客户端付费墙 | — | 自动检测 |
| 🏛️ **UAE Gov Portals** | MOHRE、FTA、DED 自动检测 | — | 始终使用 stealth 模式 |
...
1. **安装 CLI 工具** — `pip install` 装好 `agent-reach` 命令行（自动安装 scrapling）
...
5. **启动 Lightpanda**（可选）— 如果检测到 Docker，自动启动 Lightpanda 浏览器后端
6. **注册 SKILL.md** — 在 Agent 的 skills 目录安装使用指南，以后 Agent 遇到"搜推特"、"看视频"这类需求，会自动知道该调哪个上游工具
...
| 场景 | 选型 | 为什么选它 |
|------|------|-----------|
| 读网页 | [Jina Reader](https://github.com/jina-ai/reader) (Tier 1) + Scrapling fallback (Tier 2/3) + Lightpanda (Tier 4) | 多层防爬：Jina 默认快，Scrapling 反爬，Lightpanda 11x 更快 |
| 读推特 | [twitter-cli](https://github.com/public-clis/twitter-cli) | 2.1K Star，Cookie 登录，搜索/读推文/时间线/长文 |
| Reddit | [rdt-cli](https://github.com/public-clis/rdt-cli) | 304 Star，无需登录，搜索+全文+评论 |
...
### Web Fetching Tiers（网页抓取分层策略）

网页渠道采用自动降级策略，无需配置即可应对各种反爬场景：

| Tier | 方式 | 特点 | 触发条件 |
|------|------|------|---------|
| **Tier 1** | Jina Reader | 默认，最快，无需浏览器 | 普通网页 |
| **Tier 2** | Scrapling Fetcher | 隐形 Headers，轻量级 | Jina 失败时自动降级 |
| **Tier 3** | StealthyFetcher (Camoufox) | Cloudflare 绕过，JS 渲染 | `--stealth` 参数或 UAE 政府网站 |
| **Tier 4** | Lightpanda + Scrapling | 可选，11x 更快，需 Docker | 检测到 Lightpanda 服务时启用 |

**特点：**
- **自动降级**：上层失败时静默降级到下一层，无需人工干预
- **UAE 政府网站**：MOHRE、FTA、DED 等自动识别并跳过到 Stealth 模式
- **Lightpanda 可选**：Docker 运行 `lightpanda/browser:nightly`，自动检测并优先使用
